...
function checkTokenUsage() {
    ...
    fetch(url, {
        // method: "GET" // by default, fetch uses GET method
        headers: {
...
function displayStatus(json) {
    console.log(json);
    // Display the token usage information in the results div
    let pre = document.createElement("pre");
    pre.textContent = `Is Enabled: ${json.is_enabled}
    LastUsed At: ${json.last_used_at}
...
// - Add Claude's response to conversationHistory
    ...
    // add the users message to conversationHistory
    conversationHistory.push({
        "role": "user",
        "content": userInput
    });
    console.log(conversationHistory);
    // send the entire converstation to the API instead of just the current message
    ...
    .then(json => {
        displayMessage(json);
        // add the claude's response to conversationHistory
        conversationHistory.push({
        "role": "assistant",
...
function displayMessage(json) {
    ...
    // simple paragraph of the response: <p> </p>
    let para = document.createElement("p");
    para.textContent = json.content[0].text;
    results.appendChild(para);
    let para2 = document.createElement("p");
    para2.textContent = conversationHistory;
